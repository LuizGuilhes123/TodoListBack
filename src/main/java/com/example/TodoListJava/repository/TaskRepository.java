...
public interface TaskRepository extends JpaRepository<TaskEntity, Long> {
    ...
    @Query("SELECT COALESCE(MAX(t.position), -1) FROM Task t WHERE t.usuario.id = :usuarioId")
    ...
    @Query("SELECT t FROM Task t WHERE t.usuario.id = :usuarioId AND t.favorite = true ORDER BY t.position")
    List<TaskEntity> findFavoritesByUserIdOrderByPosition(@Param("usuarioId") UUID usuarioId);

    @Query("SELECT t FROM Task t WHERE t.usuario.id = :usuarioId AND t.dueDate = CURRENT_DATE ORDER BY t.position")
    List<TaskEntity> findTodayTasksByUserIdOrderByPosition(@Param("usuarioId") UUID usuarioId);

    @Query("SELECT t FROM Task t WHERE t.usuario.id = :usuarioId AND t.dueDate BETWEEN :startOfWeek AND :endOfWeek ORDER BY t.position")
    ...
    @Query("SELECT t FROM Task t WHERE t.usuario.id = :usuarioId AND YEAR(t.dueDate) = :year AND MONTH(t.dueDate) = :month ORDER BY t.position")
